#include "direct_control_module/direct_control_module.h"
...
void DirectControlModule::initialize(const int control_cycle_msec, robotis_framework::Robot *robot)
{
  ...
  /* get Param */
  module_node_->get_parameter_or("/robotis/direct_control/default_moving_time", default_moving_time_, default_moving_time_);
  module_node_->get_parameter_or("/robotis/direct_control/default_moving_angle", default_moving_angle_, default_moving_angle_);
  module_node_->get_parameter_or("/robotis/direct_control/check_collision", check_collision_, check_collision_);
...
void DirectControlModule::queueThread()
{
    ...
    /* subscribe topics */
    auto set_head_joint_sub = module_node_->create_subscription<sensor_msgs::msg::JointState>("/robotis/direct_control/set_joint_states",
...
void DirectControlModule::setJointCallback(const sensor_msgs::msg::JointState::SharedPtr msg)
{
  if (enable_ == false)
  {
    RCLCPP_INFO(module_node_->get_logger(), "Direct control module is not enable.");
  ...
  while(is_updated_ == false)
  {
    ...
    {
      RCLCPP_ERROR(module_node_->get_logger(), "present joint angle is not updated");
      return;
    }
  }
...
bool DirectControlModule::checkSelfCollision()
{
  ...
  {
    if(DEBUG == true)
      RCLCPP_INFO(module_node_->get_logger(), "Self Collision : RIGHT_ARM and BASE | %f / %f",diff_length, collision_boundary);
  ...
  {
    if(DEBUG == true)
      RCLCPP_WARN(module_node_->get_logger(), "Self Collision : RIGHT_ELBOW and BASE | %f / %f", diff_length, collision_boundary);
    collision_["r_sho_pitch"] = true;
    collision_["r_sho_roll"] = true;
  ...
  {
    if(DEBUG == true)
      RCLCPP_WARN(module_node_->get_logger(), "Self Collision : LEFT_ARM and BASE | %f / %f", diff_length, collision_boundary);
  ...
  {
    if(DEBUG == true)
      RCLCPP_WARN(module_node_->get_logger(), "Self Collision : LEFT_ELBOW and BASE | %f / %f", diff_length, collision_boundary);
    collision_["l_sho_pitch"] = true;
    collision_["l_sho_roll"] = true;
  ...
  if(collision_result == false && DEBUG == true)
    RCLCPP_WARN(module_node_->get_logger(), "============================================");
...
bool DirectControlModule::getDiff(OP3KinematicsDynamics *kinematics, int end_index, int base_index, double &diff)
{
  ...
  RCLCPP_WARN(module_node_->get_logger(), "\nBase Position [ %f,   %f , %f ] \n End Position [ %f, %f, %f ] \n Diff : %f", 
             base_position.coeff(0), base_position.coeff(1), base_position.coeff(2), end_position.coeff(0), end_position.coeff(1), end_position.coeff(2), diff );
