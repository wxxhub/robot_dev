...
void ActionModule::queueThread()
{
    ...
    done_msg_pub_ = module_node_->create_publisher<std_msgs::msg::String>("/robotis/movement_done");

    /* subscriber */
    auto action_page_sub = module_node_->create_subscription<std_msgs::msg::Int32>("/robotis/action/page_num", 
                                                                        std::bind(&ActionModule::pageNumberCallback, this, std::placeholders::_1));
    auto start_action_sub = module_node_->create_subscription<action_module_msgs::msg::StartAction>("/robotis/action/start_action",
                                                                        std::bind(&ActionModule::startActionCallback, this, std::placeholders::_1));

    /* ROS Service Callback Functions */
    auto is_running_server =  module_node_->create_service<action_module_msgs::srv::IsRunning>("/robotis/action/is_running",
                                                            std::bind(&ActionModule::isRunningServiceCallback, this, std::placeholders::_1, std::placeholders::_2, std::placeholders::_3));
...
bool ActionModule::isRunningServiceCallback(const std::shared_ptr<rmw_request_id_t> request_header,
...
void ActionModule::startActionCallback(const action_module_msgs::msg::StartAction::SharedPtr msg)
{
    if (enable_ == false)
    {
        std::string status_msg = "Action Module is not enabled";
...
void ActionModule::publishStatusMsg(unsigned int type, std::string msg)
{
  ...
  status.header.stamp = ros_clock_.now();
  status.type = type;
